...
def get_class_eng_string(class_index, score, dataset):
    class_text = dataset.eng_classes[class_index] if dataset is not None else \
        'id{:d}'.format(class_index)
    return  class_text
    # return class_text + ' {:0.2f}'.format(score).lstrip('0')


def vis_one_image_orig(
    ...
    for i in sorted_inds:
        ...
        if segms is not None and len(segms) > i:
            ...
            bin, contour, hier = cv2.findContours(e.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
            # 生成512图
            color_edge = [0, 0, 0]
            for cm in range(3):
            ...
            if classes[i] in entry['cls_list']:
                Flag = True
                cv2.putText(img_ori, text, point0, cv2.FONT_HERSHEY_SIMPLEX, 2, (250, 250, 250), 3)
                img_vis = cv2.drawContours(img_ori, contour, -1, (color_edge[2], color_edge[1], color_edge[0]), 8)
        ...
        if Flag:
            ...
            img_vis = cv2.resize(img_vis, (512, 512), interpolation=cv2.INTER_CUBIC)
            cv2.imwrite(os.path.join(output_dir, '{}'.format(output_name)), img_vis)
        # plt.close('all')
