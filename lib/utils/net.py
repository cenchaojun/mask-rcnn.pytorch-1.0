...
def _CorrectMomentum(optimizer, param_keys, correction):
    ...
    for p_key in param_keys:
        # if 'momentum_buffer' not in optimizer.state[p_key].keys(): # LJ very important
        #     # pdb.set_trace()     # we should add parameter "mean_vect" and "covar_matrix" into optimizer.params
        #     continue
...
def load_ckpt(model, ckpt):
    """Load checkpoint"""
    mapping, _ = model.detectron_weight_mapping # LJ: remove detectron mapping
    # note: we build new model without detectron mapping, so remove it
    state_dict = {}
    for name in ckpt:
        # pdb.set_trace()
        #if mapping[name]:
        ...
        state_dict[name] = ckpt[name]
    model.load_state_dict(state_dict, strict=False) # LJ: only load the existing dic of the model
