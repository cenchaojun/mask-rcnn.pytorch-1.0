...
import datasets.hospital_dataset_evaluator as hospital_dataset_evaluator
...
def evaluate_mAP(dataset, roidb, all_boxes, output_dir):
    """LJY: compute recall and FPs for each category"""
    logger.info('Evaluating detections')
    coco_eval = hospital_dataset_evaluator.evaluate_mAP(
        dataset, roidb, all_boxes, output_dir
    )
...
def evaluate_mAP_combine(datasets, roidbs, all_boxes_list, output_dir):
    """LJY: compute recall and FPs for each category"""
    logger.info('Evaluating detections')
    coco_eval = hospital_dataset_evaluator.evaluate_mAP_combine(
        datasets, roidbs, all_boxes_list, output_dir
    )
    box_results = _coco_eval_to_box_results(coco_eval)
    return OrderedDict([(dataset.name, box_results)])


def evaluate_boxes_recall_and_FP(dataset, roidb, all_boxes, output_dir):
    """LJY: compute recall and FPs for each category"""
    logger.info('Evaluating detections')
    coco_eval = hospital_dataset_evaluator.evaluate_boxes_recall_and_FP(
        dataset, roidb, all_boxes, output_dir
    )
    box_results = _coco_eval_to_box_results(coco_eval)
    return OrderedDict([(dataset.name, box_results)])
...
def evaluate_class_AUC(dataset, roidb, all_boxes, output_dir):
    """LJY: AUC for each category"""
    logger.info('Evaluating detections')
    coco_eval = hospital_dataset_evaluator.evaluate_class_AUC(
        dataset, roidb, all_boxes, output_dir
    )
    box_results = _coco_eval_to_box_results(coco_eval)
    return OrderedDict([(dataset.name, box_results)])
...
def evaluate_boxes(dataset, all_boxes, output_dir, use_matlab=False):
    ...
    if _use_hospital_dataset_evaluator(dataset):
        coco_eval = hospital_dataset_evaluator.evaluate_boxes(
...
def _use_hospital_dataset_evaluator(dataset):
    """Check if the dataset uses the general json dataset evaluator."""
    return dataset.name.find('hospital') > -1
