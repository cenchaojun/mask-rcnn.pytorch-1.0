...
class JsonDataset(object):
    ...
    def __init__(self, name):
        ...
        categories = ['肺门增大', '纵隔增宽', '肺实变', '肺不张', '三均匀粟粒样结节', '非三均匀粟粒样结节',
                      '空洞', '索条影', '卫星灶', '结核结节', '结核球', '钙化', '毁损肺', '胸膜增厚',
                      '胸腔积液', '肋膈角变钝'
                      ]  # 16

        chi2eng = {'肺实变': 'consolidation', '索条影': 'fibrosis',  '胸腔积液': 'effusion',
                   ...
                   '肩关节异常': 'shoulder_abnormal', '索条影': 'stripe', '三均匀粟粒样结节': 'uniform_diffusive_nodule',
                   '非三均匀粟粒样结节': 'non_uniform_diffusive_nodule', '卫星灶': 'satellitic_lesion',
                   '结核结节': 'PB_nodule', '结核球': 'PB_mass', '毁损肺': 'destroyed_lung',
                   '肋膈角变钝': 'stophrenic_angle_obtused', '肺门增大': 'hilum_increase',
                   '纵隔增宽': 'widened_mediastinal'
                   }
