...
def evaluate_mAP(json_datasets, roidbs, all_boxes_list, output_dir, cleanup=False):
    ...
    all_boxes: num_cls x num_images x [num_boxes x 5]
    """
    mAP_folder = '/home/lianjie/mask-rcnn.pytorch-1.0/mAP'
    # LJ 更新文件夹
    gt_dir = osp.join(mAP_folder, 'ground-truth')
    if os.path.exists(gt_dir):
        shutil.rmtree(gt_dir)

    pre_dir = osp.join(mAP_folder, 'predicted')
    if os.path.exists(pre_dir):
        shutil.rmtree(pre_dir)

    for name in ['ground-truth','predicted']:
        dir_path = os.path.join(mAP_folder,name)
    ...
    if tp_flag:
        gt_numpy = np.zeros(len(roidb))
        pred_numpy = np.zeros(len(roidb))
        gt_pos, tp, fp = 0, 0, 0

    for i, entry in enumerate(roidb):  # for each image
        #print(i, entry['eva_id'], entry['fold'], entry['file_name'])
        if i % 100 == 0:
            print('%d files have been test!!!' %i)
        ...
        for cls_ind, cls in enumerate(json_dataset.classes):  # for each cls
            ...
            if tp_flag:
                if len(gt_classes) > 0:
                    gt_numpy[i] = gt_classes.max()
                else:
                    gt_numpy[i] = 0
                    # print(entry['eva_id'])
                if len(dets) > 0:
                    pred_numpy[i] = dets[:, -1].max()
                    # pdb.set_trace()
                else:
                    pred_numpy[i] = 0
                if gt_numpy[i] == 1:
                    gt_pos += 1
                    if pred_numpy[i] > tp_thr:
                        tp += 1
                else:
                    if pred_numpy[i] > tp_thr:
                        fp += 1
    ...
    if tp_flag:
        print('sample number ', len(roidb))
        print('pos gt', gt_pos)
        print('tp', tp)
        print('fp', fp)

            # # LJ
            # vis_out_dir = '/home/lianjie/mask-rcnn.pytorch-1.0/mAP/vis/'
            # if gt_boxes.shape[0] > 0:  # then we draw gt box and pred boxes
            #     im = cv2.imread(entry['image'])
            #     more_text = str(entry['eva_id']) + ' ' + entry['doc_name']
            #     im = vis_boxes_ljy(im, gt_boxes, dets[:, :-1], more_text)
            #     if not os.path.exists(vis_out_dir + cls):
            #         os.makedirs(vis_out_dir + cls)
            #
            #     out_path = os.path.join(vis_out_dir, cls, entry['file_name'])
            #     cv2.imwrite(out_path, im)
            # # LJ 没有gt的类 却有pred 假阳
            ...
            #     im = vis_boxes_ljy(im, gt_boxes, dets[:, :-1], more_text)
            #
            #     if not os.path.exists(vis_out_dir + cls + '_fp'):
            #         os.makedirs(vis_out_dir + cls + '_fp')
            #
            #     out_path = os.path.join(vis_out_dir, cls + '_fp', entry['file_name'])
            #     cv2.imwrite(out_path, im)
