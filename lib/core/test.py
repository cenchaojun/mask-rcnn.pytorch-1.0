...
# LJ 添加roidb信息
def im_detect_all(model, im, roidb, box_proposals=None, timers=None):
    ...
    if cfg.TEST.BBOX_AUG.CROP4:  # LJY
        rows, cols = im.shape[0], im.shape[1]
        mid_row, mid_col = rows // 2, cols // 2
        xy_4 = [[0, 0, mid_col + 50, mid_row + 50],  # x1, y1, x2, y2
                [mid_col - 50, 0, cols, mid_row + 50],
                [0, mid_row - 50, mid_col + 50, rows],
                [mid_col - 50, mid_row - 50, cols, rows]]
        ...
        for i in range(4):
            xyxy = xy_4[i]
            im_part = im[xyxy[1]: xyxy[3], xyxy[0]: xyxy[2], :]
            if cfg.TEST.BBOX_AUG.ENABLED:
                scores, boxes, im_scale, blob_conv = im_detect_bbox_aug(  # LJY: call the function below
                    model, im_part, box_proposals)
            else:
                scores, boxes, im_scale, blob_conv = im_detect_bbox(
                    model, im_part, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
            boxes[:, 0::2] += xyxy[0]
            boxes[:, 1::2] += xyxy[1]
            # boxes[:, 2] += xyxy[0]
            # boxes[:, 3] += xyxy[1]

            add_preds_t(scores, boxes)
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        # pdb.set_trace()
        # LJ 默认模式 进入此函数 添加roidb信息
        scores, boxes, im_scale, blob_conv = im_detect_bbox(
            model, im, roidb, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
    # pdb.set_trace()

    if cfg.TEST.BBOX_AUG.CROP4:  # LJY: add the results from the original image
        add_preds_t(scores, boxes)
        boxes = np.vstack(boxes_ts)
        scores = np.vstack(scores_ts)
...
def im_conv_body_only(model, im, target_scale, target_max_size):
    # LJ 添加新的关键字
    inputs, im_scale = _get_blobs(im, None, None, target_scale, target_max_size)
...
# LJ 添加 roidb 信息 用于测试模式的part信息读取
def im_detect_bbox(model, im, roidb, target_scale, target_max_size, boxes=None):
    """Prepare the bbox for testing"""

    inputs, im_scale = _get_blobs(im, boxes, roidb, target_scale, target_max_size)
    # pdb.set_trace()
    ...
    # pdb.set_trace()
    # LJ inputs ['data', 'imfo', 'im', 'part_boxes']
    # LJ 在此处测试时间
    # global all_time
    # s_time = time.time()
...
def im_detect_bbox_aug(model, im, box_proposals=None):
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        scores_scl, boxes_scl = im_detect_bbox_scale(  # N x cls, N x cls*4
...
# LJ 增加roidb信息
def _get_blobs(im, rois, roidb, target_scale, target_max_size):
