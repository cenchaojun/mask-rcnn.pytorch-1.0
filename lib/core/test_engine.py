...
from datasets.hospital_dataset import JsonDataset
...
def run_inference(
    ...
    def result_getter():
        if is_parent:
            ...
            task_evaluation.evaluate_mAP(datasets, roidbs, all_boxes_list, output_dir)
            #pdb.set_trace()
            #return all_results 
        else:
...
def test_net_on_dataset(  # parent_func
        args,
        dataset_name,
        proposal_file,
        output_dir,
    ...
    if dataset_name == 'hospital_test':
        from datasets.hospital_dataset import JsonDataset
    elif dataset_name == 'quality_control_test':
        from datasets.quality_dataset import JsonDataset
    elif dataset_name == 'PB_test':
        from datasets.PB_dataset import JsonDataset
    ...
    if multi_gpu:
        ...
        all_boxes, all_segms, all_keyps = multi_gpu_test_net_on_dataset(  # call the function below
    ...
    return dataset, roidb, all_boxes

    # task_evaluation.evaluate_mAP(dataset, roidb, all_boxes, output_dir)
    # task_evaluation.evaluate_boxes_recall_and_FP(dataset, roidb, all_boxes, output_dir)
    # task_evaluation.evaluate_class_AUC(dataset, roidb, all_boxes, output_dir)
...
def multi_gpu_test_net_on_dataset(args, dataset_name, proposal_file, num_images, output_dir):
    ...
    for det_data in outputs:
        ...
        for cls_idx in range(1, cfg.MODEL.NUM_CLASSES):  # exclude 'background' class here
...
def test_net(  # child func
    ...
    for i, entry in enumerate(roidb):
        ...
        # LJ 添加part信息 用于测试模式下的 part信息读取
        cls_boxes_i, cls_segms_i, cls_keyps_i = im_detect_all(model, im, roidb_single, box_proposals, timers)
