...
# LJY: whether to use box regression
__C.TRAIN.USE_REGRESSION = True

# LJY: classification weight
__C.TRAIN.CLS_WEIGHT = 3.0
...
# During training, all proposals specified in the file are used (no limit is applied)
...
__C.TRAIN.SNAPSHOT_ITERS = 6000 # 10000 /gpu_num 多少步保存一个模型 gk_yy 6000 两个epoch
...
# (anchor, gt box) pair to be a positive example (IOU >= thresh ==> positive RPN example)
__C.TRAIN.RPN_POSITIVE_OVERLAP = 0.7

# Maximum overlap allowed between an anchor and ground-truth box for the
# (anchor, gt box) pair to be a negative examples (IOU < thresh ==> negative RPN example)
...
__C.TEST.SCORE_THRESH = 0.02

# Save detection results files if True
# If false, results files are cleaned up (they can be large) after local evaluation
...
# LJY: Enable test-time crop (equally into 4 parts) for bounding box detection if True
__C.TEST.BBOX_AUG.CROP4 = False
...
# Hyperparameter used by the scoring method (it has different meanings for different methods)
...
# Use a class agnostic bounding box regressor instead of the default per-class regressor
...
#    if the Faster R-CNN model was trained with stage-wise training (which
...
# LJY: use different weights on classes, e.g. small weight on the bg class
__C.FAST_RCNN.USE_CLS_WEIGHT = False
...
# LJY: 'ce' for the original loss, 'focal' for focal loss, use 'sigmoid' activation when using 'focal'
__C.RPN.LOSS = 'ce'
...
# Use class specific mask predictions if True (otherwise use class agnostic mask predictions)
...
# Choices: [0, 2, 3, 4, 5]. O means not fixed. First conv and bn are defaults to be fixed.
...
# LJY: Downsample 8 times at conv4
__C.RESNETS.DOWN8 = False
...
# Drop negative images
__C.DROP_NEGATIVE_IMAGES = False

# Whether use histogram equalization
__C.HISTO_EQUAL = False
...
# Clip bounding box transformation predictions to prevent np.exp from overflowing
...
# Output basedir
# __C.OUTPUT_DIR = 'Outputs' for saving model
__C.OUTPUT_DIR = '/home/lianjie/mask-rcnn.pytorch-1.0/jeline_output'
...
# relation module
__C.USE_CLS_RELA = False
__C.USE_SPA_RELA = False
__C.USE_CONTEXT_RELA = False
__C.USE_INNER_CONTEXT_RELA = False
__C.USE_LUNG_FEATURE = False
__C.INNER_ROI_SIZE = [1,1]
__C.USE_DCN = False
__C.INNERPLANES_LIST = [128, 256, 512]
__C.USE_CASCADE = False
__C.USE_CASCADE_REG_CLASSES = False
# ---------------------------------------------------------------------------- #
