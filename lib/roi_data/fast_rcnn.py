...
# LJY: called by GenerateProposalLabelsOp in generate_proposal_labels.py
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    bg_inds = np.where((max_overlaps < cfg.TRAIN.BG_THRESH_HI) &  # [0.0, 0.5) by default
    ...
    if 'bbox_targets' not in roidb:
        ...
        if len(gt_inds) > 0:  # LJY
            ...
            # pdb.set_trace()
            bbox_targets = _compute_targets(sampled_boxes, gt_boxes, sampled_labels)
            bbox_targets, bbox_inside_weights = _expand_bbox_targets(bbox_targets)
            # pdb.set_trace()
    else:
        # LJ 不会进入
...
def _add_multilevel_rois(blobs):
    ...
    according the level assignment heuristic (see: modeling.FPN.map_rois_to_fpn_levels).
    """
