...
def add_rpn_blobs(blobs, im_scales, roidb):  # LJY: called by minibatch.py
    ...
    if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_RPN:
        ...
        foa = data_utils.get_field_of_anchors(cfg.RPN.STRIDE, cfg.RPN.SIZES, cfg.RPN.ASPECT_RATIOS)
    ...
    valid_keys = [
        ...
        'has_mask', 'image', 'parts_list', 'part_boxes' # LJ add image parts keys
    ]
...
def _get_rpn_blobs(im_height, im_width, foas, all_anchors, gt_boxes):
    ...
    if straddle_thresh >= 0:
        ...
        # Set TRAIN.RPN_STRADDLE_THRESH to -1 (or a large value) to keep all anchors
    ...
    num_fg = int(cfg.TRAIN.RPN_FG_FRACTION * cfg.TRAIN.RPN_BATCH_SIZE_PER_IM)  # 0.5 * 256 by default
    fg_inds = np.where(labels == 1)[0]
    if len(fg_inds) > num_fg:
        disable_inds = npr.choice(
            fg_inds, size=(len(fg_inds) - num_fg), replace=False
        )
        labels[disable_inds] = -1  # un-used fg, so don't care
    fg_inds = np.where(labels == 1)[0]

    # subsample negative labels if we have too many samples with replacement,
    # but since the set of bg inds is large most samples will not have repeats
    ...
    if len(bg_inds) > num_bg:
        ...
        labels[enable_inds] = 0  # bg
    bg_inds = np.where(labels == 0)[0]  # LJY: not used anymore

    bbox_targets = np.zeros((num_inside, 4), dtype=np.float32)

    if len(gt_boxes) > 0:  # LJY
    ...
    num_examples = np.sum(labels >= 0)  # LJY: fg & bg

    if num_examples > 0:  # LJY
    ...
    start_idx = 0
    for foa in foas:
        H = foa.field_size  # e.g. 84
        W = foa.field_size  # e.g. 84
        A = foa.num_cell_anchors  # e.g. 15
