...
import pdb
from modeling.dcn.layers import ConvOffset2D
from modeling.dcn_mmdet import DeformConv
...
class ResNet_roi_conv5_head(nn.Module):
    def __init__(self, dim_in, roi_xform_func, spatial_scale):
        ...
        if cfg.USE_CONTEXT_RELA:
            # 可学习矩阵
            self.matrix = torch.nn.Parameter(torch.randn((14, 14)))
    ...
    def forward(self, x, rpn_ret, part_rois, cascade=False, rois=None):
        ...
        # 针对c4模式 增加cascade操作
            ...
            cat_x = ROIAlign((resolution, resolution),
...
class bottleneck_transformation(nn.Module):
    ...
    # resnet50: dim_in [64, 256, 512, 1024]
    ...
    def __init__(self, inplanes, outplanes, innerplanes, stride=1, dilation=1, group=1,
        ...
        if cfg.USE_DCN and innerplanes in cfg.INNERPLANES_LIST:
            ...
            self.conv2_offset = nn.Conv2d(innerplanes, deformable_groups * offset_channels,
                kernel_size=3, stride=str3x3, padding=dilation, dilation=dilation)
            self.conv2 = conv_op(innerplanes, innerplanes, kernel_size=3,
                stride=str3x3, padding=dilation, dilation=dilation,
    ...
    def _init_weights(self):
        torch.nn.init.normal_(self.conv2_offset.weight, mean=0.0, std=0.01)
        torch.nn.init.constant_(self.conv2_offset.bias, 0)
