...
# from pytorch_semseg_master.segment import segment
...
def main():
    ...
    for i in xrange(num_images):  # for each image
        ...
        for cls in range(1, num_class):  # for each cls
            ...
            if dets.shape[0] == 0:
                continue
            dets_extend = np.pad(dets, ((0, 0), (0, 1)),  # add 0 rows above, below and left, but 1 row right
                                 mode='constant', constant_values=cls)  # append cls to dets
        ...
        for idx in range(n):  # for each region
            ...
            if th > th_cls[cls]:  # diagnosed to have the sym
                ...
                final_results.append({'name': ename, 'type': _type, 'box': list(nms_dets[idx, 0:4])})
