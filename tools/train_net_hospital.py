...
def parse_args():
    ...
    parser.add_argument(
        ...
        action='store_true')
    parser.add_argument(
        '--output_dir', help='path to the saved ckpt file')
...
def main():
    """Main function"""
    dir = '/home/lianjie/mask-rcnn.pytorch-1.0/jeline_output/'
    cache_path = os.path.abspath(os.path.join(dir, 'cache'))
    cache_filepath = os.path.join(cache_path, 'hospital_train_gt_roidb.pkl')
    ...
    if args.dataset == "hospital":
        cfg.TRAIN.DATASETS = ('hospital_train',)
        # cfg.TRAIN.DATASETS = ('hospital_train', 'hospital_train_part', 'hospital_train_small')
        # cfg.TRAIN.DATASETS = ('hospital_train_small', )
        cfg.MODEL.NUM_CLASSES = 13 + 1  # !!!
    elif args.dataset == 'PB':
        cfg.TRAIN.DATASETS = ('PB_train',)
        cfg.MODEL.NUM_CLASSES = 16 + 1  # !!!
    ...
    #     if roidb[i]['eva_id'] == 67523:
    #         pdb.set_trace()
        # for flag in roidb[i]['is_crowd'].tolist():
        #     if not flag:
        #         pdb.set_trace()

    ### Model ###
    maskRCNN = Generalized_RCNN()  # LJY: initialize the network
    # for key, vaule in maskRCNN.named_parameters(): # LJ output required_grad parameter
    #     print(key)
