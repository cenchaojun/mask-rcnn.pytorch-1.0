...
ground_truth_files_list = glob.glob('ground-truth-yang/*.txt')
...
for txt_file in ground_truth_files_list:
  ...
  if not os.path.exists('predicted-yang/' + file_id + ".txt"):
...
predicted_files_list = glob.glob('predicted-yang/*.txt')
...
for class_index, class_name in enumerate(gt_classes):
  ...
  for txt_file in predicted_files_list:
    ...
    file_id = os.path.basename(os.path.normpath(file_id))
    if class_index == 0:
      if not os.path.exists('ground-truth-yang/' + file_id + ".txt"):
