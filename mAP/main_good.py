...
ground_truth_files_list = glob.glob('ground-truth-good/*.txt')
...
for txt_file in ground_truth_files_list:
  ...
  if not os.path.exists('predicted-good/' + file_id + ".txt"):
...
predicted_files_list = glob.glob('predicted-good/*.txt')
...
with open(results_files_path + "/results.txt", 'w') as results_file:
  ...
  for class_index, class_name in enumerate(gt_classes):
    ...
    # found = False
    # for idx, _ in enumerate(fp):
    #     if _ >= 1612 * 0.1:  # 1638 * 0.2 = 327.6, 371 * 0.2 = 74.2, 1876 * 0.2 = 375.2, 1876*4*0.2=1500.8
    #         print("%.3f" % (fp[idx] / 1612))
    ...
    # print(len(rec), len(prec), len(tp), len(fp))  # N, N, N-2, N-2

    # print('precision')
    # print("%.3f" % (prec[-2]))

    # print('FP per image at max recall')
    # if len(fp) > 0:
    #   print("%.3f" % (fp[-1] / (1876*4)))
    # else:
    #   print()
